...
data {
  ...
  // switches
  int<lower=1, upper=3> model_variant; // varying K, tK or both
  int<lower=0, upper=1> cen;           // censoring
}
parameters{
  ...
  // carrying capacities
  vector[N_pop] log_pK_raw;
  vector[N_grp] log_mu_p;
...
transformed parameters {
  ...
  vector[N_pop] p0 = exp(log_mu_p[grp_pop] + log_p0_raw * sigma_log_p);
  vector[N_pop] pK = exp(log_mu_p[grp_pop] + log_pK_raw * sigma_log_p);
