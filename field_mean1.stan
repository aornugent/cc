functions {
  ...
  vector beta_curve(vector x0, vector xK, 
    ...
    vector[n] xt = log(xK - x0) + log(1 + (tK - t) ./ (tK - tmax)) + 
                    log(t ./ tK) .* (tK ./ (tK - tmax));
...
transformed parameters {
    ...
    // vector[N_pop] p0 = mu_p[grp_pop] + p0_raw * sigma_p;
    // vector[N_pop] pK = mu_p[grp_pop] + pK_raw * sigma_p;
...
model{
  {
    ...
    delta_log_e = (1 - delta_gm) ./ (1 - delta) .* sigma_log_e[grp];
    //delta_log_e = sigma_log_e[grp];
